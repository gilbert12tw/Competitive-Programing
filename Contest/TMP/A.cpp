...
inline void solve() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 1; j < (1<<t); j++) {
      ...
      for (int k = 0; k < t; k++) {
        if (!get_bit(j, k)) {
          vector<string> nxt = tmp;
          nxt.eb(s[i][k]);
          sort(ALL(nxt));
          if (mp[nxt] == 0) mp[nxt] = ++cnt;
          int id = mp[nxt];
          dinic.add_edge(2 * u - 1, 2 * id, 1);
        }
      }
  ...
  // 2 * i     => out
  for (int i = 1; i <= cnt; i++) {
    dinic.add_edge(2 * i, 2 * cnt + 1, 1);
    dinic.add_edge(0, 2 * i - 1, 1);
  }

  cout << cnt - dinic.max_flow(0, 2 * cnt + 1);
}
